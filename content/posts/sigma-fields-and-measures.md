...
7. Let $A$ be a Borel set. Show that the $\sigma$-field of subsets of $A$ containing the open sets in $A$ is - $$\set{B\in{\Bbb{B}(\reals) | B\subset{A}}}=\Bbb{B}(\reals)\cap{A}$$
...
__Answer__:
Let $\gamma$ be the collection of open sets in $\Reals$ and $A$ be the Borel set in question. We must show that $\sigma_A(\gamma\cap{A}) = \Bbb{B}(\reals)\cap{A}$. 

First, we will show that $\Bbb{B}(\reals)\cap{A}$ is a $\sigma$-field. For if $a,b\in\Bbb{B}(\reals)$ we have $a\cap{A}, b\cap{A}\in\Bbb{B}(\reals)\cap{A}$, but since $\Bbb{B}(\reals)$ is a $\sigma$-field we have $(a\cap{A})\cup{}(b\cap{A})=(a\cup{b})\cap{A}\in\Bbb{B}(\reals)\cap{A}$ implying $\Bbb{B}(\reals)\cap{A}$ is indeed a $\sigma$-field. 

Indeed, we have $\gamma\cap{A}\subset\Bbb{B}(\reals)\cap{A}\implies\sigma_A(\gamma\cap{A})\subset\Bbb{B}(\reals)\cap{A}$.

To show the converse, $\Bbb{B}(\reals)\cap{A}\subset\sigma_A(\gamma\cap{A})$, we will use the "Good Set" principle.

Define - $$M = \set{X\in\Bbb{B}(\reals) | X\cap{A} \in \sigma_A(\gamma\cap{A})}$$
...
We finish the proof by observing that $\gamma\in{M}$ but since $M$ is a $\sigma$-field we also have $\sigma(\gamma)=\Bbb{B}(\reals)\in{M}.\space \square$ 